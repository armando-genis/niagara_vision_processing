...
pcl::PointCloud<pcl::PointXYZ> PointsDetector::euclideanClusterExtraction(const pcl::PointCloud<pcl::PointXYZ>& cloud)
{
    ...
    for (const auto& it : cluster_indices)
    {
        for (const auto& pit : it.indices)
            clustered_cloud.points.push_back(cloud_ptr->points[pit]);
    }
...
void PointsDetector::createBoundingBox(vision_msgs::msg::Detection3DArray& detections3d_msg, const pcl::PointCloud<pcl::PointXYZ>& cloud, const std::vector<vision_msgs::msg::ObjectHypothesisWithPose>& detection_results)
{
    pcl::PointXYZ min_pt, max_pt;
    pcl::getMinMax3D(cloud, min_pt, max_pt);

    vision_msgs::msg::BoundingBox3D bbox;
    bbox.center.position.x = (max_pt.x + min_pt.x) / 2;
    bbox.center.position.y = (max_pt.y + min_pt.y) / 2;
    bbox.center.position.z = (max_pt.z + min_pt.z) / 2;
    ...
    detection3d.bbox = bbox;
    detection3d.results = detection_results;
...
std::tuple<vision_msgs::msg::Detection3DArray, sensor_msgs::msg::PointCloud2> PointsDetector::projectCloud(const pcl::PointCloud<pcl::PointXYZ>& cloud, const vision_msgs::msg::Detection2DArray::SharedPtr& detections2d_msg, const std_msgs::msg::Header& header)
{
    ...
    return std::make_tuple(detections3d_msg, combine_detection_cloud_msg);  // Added return statement
